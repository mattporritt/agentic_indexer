...
_RUNTIME_CONTRACT_COMMANDS = frozenset({"find-definition", "semantic-context", "build-context-bundle"})
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        payload = _dispatch_command(args)
    except IndexerError as exc:
        if _supports_runtime_contract(args):
...
def run_find_symbol(db_path: str, symbol: str) -> dict:
    ...
    connection = _open_index_database(db_path)
    try:
...
def _dispatch_command(args: argparse.Namespace) -> dict:
    """Return the payload for one parsed CLI command."""

    if args.command == "index":
        return run_index(args.moodle_path, args.db_path, args.workers)
    if args.command == "find-symbol":
        return run_find_symbol(args.db_path, args.symbol)
    if args.command == "find-definition":
        return run_find_definition(
            args.db_path,
            args.symbol,
            args.type,
            args.limit,
    ...
    if args.command == "find-related-definitions":
        return run_find_related_definitions(args.db_path, args.symbol, args.file, args.limit)
    if args.command == "suggest-edit-surface":
        return run_suggest_edit_surface(args.db_path, args.symbol, args.file, args.limit)
    if args.command == "dependency-neighborhood":
        return run_dependency_neighborhood(args.db_path, args.symbol, args.file, args.limit)
    if args.command == "semantic-context":
        return run_semantic_context(
            args.db_path,
            args.symbol,
            args.file,
            args.query,
    ...
    if args.command == "propose-change-plan":
        return run_propose_change_plan(args.db_path, args.symbol, args.file, args.query, args.limit)
    if args.command == "assess-test-impact":
        return run_assess_test_impact(args.db_path, args.symbol, args.file, args.query, args.limit)
    if args.command == "execution-guardrails":
        return run_execution_guardrails(args.db_path, args.symbol, args.file, args.query, args.limit)
    if args.command == "build-context-bundle":
        return run_build_context_bundle(
            args.db_path,
            args.symbol,
            args.file,
            args.query,
    ...
    if args.command == "file-context":
        return run_file_context(args.db_path, args.file)
    if args.command == "component-summary":
        return run_component_summary(args.db_path, args.component)
    if args.command == "suggest-related":
        return run_suggest_related(args.db_path, args.file)
    raise IndexerError(f"Unsupported command: {args.command}")


def _open_index_database(db_path: str):
    """Open the SQLite index using the CLI's canonical path normalization."""

    return open_database(Path(db_path).expanduser().resolve())


def _supports_runtime_contract(args: argparse.Namespace) -> bool:
    """Return whether this invocation should emit the runtime error envelope."""

    return bool(getattr(args, "json_contract", False) and args.command in _RUNTIME_CONTRACT_COMMANDS)
